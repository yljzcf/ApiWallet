...
(async () => {
...

  {
    ...
    assert.strictEqual(initialBoard.cards[0].name, '旧组名', '自定义编组名应优先显示');
...

  {
    const storage = createStorageMock({
      ...
      boardData: { 'custom-a': '1.00', 'custom-b': '2.00' },
      groups: [{ id: 'group:1', taskIds: ['custom-a', 'custom-b'] }],
      manualOrder: ['group:1']
    });
    const editor = testExports.createBoardEditorController({
      storage,
    ...
    const board = await editor.loadDraft();
    assert.strictEqual(board.cards[0].name, '站点 A + 站点 B', '配置页未命名编组应使用站点名拼接，保持与插件面板一致');
  }
